...
void Sphere::generateVertices() {
    ...
    for(int i = 0; i <= stacks; ++i) {
        ...
        for(int j = 0; j <= sectors; ++j) {
            ...
            colors.push_back(0.5f + 0.5f * nx);  // Color based on normal
...
void Sphere::updatePosition(glm::vec3 position) {
...
bool Sphere::checkCollision(const Sphere& otherSphere) {
    ...
    std::cout << this->position.x <<" , "  <<otherSphere.position.x << " , " <<std::endl;
...
void Sphere::moveSphere(float deltaTime, float velocity) {
    position.x += velocity * deltaTime;  // Move sphere along x-axis based on velocity and time delta
}

void Sphere::rotateSphere(float deltaTime) {
    angle += 45.0f * deltaTime;  // Rotate by 45 degrees per second (you can adjust the speed here)
    if (angle > 360.0f) {
        angle -= 360.0f;
    }
}
